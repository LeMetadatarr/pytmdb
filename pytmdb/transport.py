...
- **Opt-in disk caching** — GET/HEAD responses with status ``200`` are stored
  on disk when ``PYTMDB_HTTP_CACHE`` is set, using stdlib only (hashlib,
  json, base64).
...
``PYTMDB_HTTP_CACHE``
    ...
    ``~/.cache/pytmdb/http`` is used.

``PYTMDB_HTTP_CACHE_TTL``
...
LOG = logging.getLogger("pytmdb.transport")

_DEFAULT_CACHE_DIR = Path.home() / ".cache" / "pytmdb" / "http"
_DEFAULT_TTL = 86400
...
def _cache_from_env() -> Optional[DiskCache]:
    ...
    env = os.environ.get("PYTMDB_HTTP_CACHE", "").strip()
    if not env:
        return None
    ttl_raw = os.environ.get("PYTMDB_HTTP_CACHE_TTL", str(_DEFAULT_TTL)).strip()
    try:
        ttl_val = int(ttl_raw)
        ttl: Optional[int] = ttl_val if ttl_val > 0 else None
    except ValueError:
        LOG.warning("Invalid PYTMDB_HTTP_CACHE_TTL %r — using default", ttl_raw)
        ttl = _DEFAULT_TTL
...
def make_cf_session():
    """Return an anti-bot session for hosts guarded by Cloudflare (TMDB has
    been observed behind Cloudflare from some networks).

    Prefers the org's :class:`unblock_requests.CloudflareSession` (curl_cffi
    impersonation) when the optional ``[stealth]`` extra is installed;
    otherwise falls back to a plain :func:`make_session` so the base install
...
def clear() -> int:
    ...
    d = _resolve_dir(os.environ.get("PYTMDB_HTTP_CACHE", ""))
    if not d.is_dir():
        return 0
    count = 0
